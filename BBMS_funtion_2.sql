...
BEGIN 
    ...
    FOR cursor1 IN (select blood_bank_id,blood_bank_name,blood_bank_contact_no,blood_bank_address from BloodBank where blood_bank_id in (select blood_bank_id from Donate where donar_id in (select donar_id from donar where blood_group = (select blood_group from Patient where patient_id=pid)) ) and blood_bank_address in (select patient_address from Patient where patient_id=1001))
          LOOP
            DBMS_OUTPUT.PUT_LINE('id: ' || cursor1.blood_bank_id ||
                               ', blood bank name: ' || cursor1.blood_bank_name||
                               ', contact no: ' || cursor1.blood_bank_contact_no);
